...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	// Parse from body of request to get a json object.
	lat, _ := strconv.ParseFloat(r.FormValue("lat"), 64) // get from form data
	...
	if p.Type == "image" {
		uri := fmt.Sprintf("gs://%s/%s", BUCKET_NAME, id) // Sprintf - print the value to uri (instead of console)
